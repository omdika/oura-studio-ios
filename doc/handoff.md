...
| v3.5 | 2026-08-12 | Frontend+Backend | **PLANNED: Multi-fabric PatternSpec — satu resep untuk semua kain (belum diimplementasikan).** Arsitektur lama menyimpan satu PatternSpec per kain: multi-kain disimulasikan dengan N PatternSpec berbagi `productSizeId` ("gabungkan" mode). Ini menyebabkan tiga bug fundamental untuk produk multi-kain seperti pouch: (1) **Stok salah** — setiap konfirmasi batch menambah stok secara independen, pouch bertiga kain tercatat +stok 3× padahal baru 1 pouch jadi; (2) **HPP tidak lengkap** — setiap PatternSpec hanya tahu biaya kainnya sendiri, biaya kain lain hilang dari kalkulasi; (3) **Optimizer buta** — tidak bisa tunjukkan bottleneck ("Roll A cukup 15 pouch tapi Roll B hanya 7"). **Arsitektur baru:** satu PatternSpec = satu resep lengkap untuk satu ukuran produk, berisi N fabric layers. Backend: tabel join baru `pattern_spec_fabric (id, pattern_spec_id, material_id, cut_width_cm, cut_height_cm, rotation_allowed, fabric_label?)` — field `fabric_material_id/cut_width_cm/cut_height_cm` dipindah dari `pattern_spec` ke tabel ini. **Frontend yang berubah:** (1) TambahResepSheet + TambahProdukLengkapSheet: hapus toggle Gabungkan/Pisah — semua kain yang dipilih langsung masuk satu PatternSpec, POST sekali; (2) ResepEditorView: edit N fabric layers dalam satu form; (3) OptimasiView: ketika PatternSpec punya >1 kain, tampil panel "Pilih roll per kain" + kalkulasi bottleneck (max pouch = min kapasitas semua roll); (4) ProduksiBatch: satu batch mencakup N fabric cuts sekaligus, konfirmasi sekali. **Frontend yang tidak berubah:** produk 1 kain (scrunchie) — UX identik, hanya data di-store ke tabel join alih-alih field flat. **Pisah mode** (fabric variant = stok terpisah) tetap via "Tambah Varian" di ProdukDetailView — bukan urusan resep. Lihat backend v2.15 untuk spec schema dan migration. |
