...
| v3.44 | 2026-08-29 | Frontend | **IMPLEMENTED: Filter Rentang Tanggal Update Stok (Updated Date).** Menambahkan filter rentang tanggal ("Tanggal Update" menggunakan komponen DateRangeField) di halaman Daftar Produk (ProdukListView) dan Generator QR (QRGeneratorView). Di halaman Daftar Produk, filter membatasi tampilan hanya untuk produk dengan penambahan stok di periode tersebut beserta rincian jumlah penambahan (+N masuk). Di halaman Generator QR, filter membatasi tampilan varian ukuran dan mempra-isi jumlah cetak QR (pre-fill) sesuai dengan kuantitas stok yang baru ditambahkan di rentang tanggal tersebut. Rincian spesifikasi: `doc/versions/v3.44.md`. |
| v3.43 | 2026-08-28 | Frontend | **IMPLEMENTED: Perbaikan Keterangan/Caption QR Code Cetak.** Memperbaiki masalah pemotongan teks (*clipping*) pada keterangan (*caption*) QR label yang dicetak agar menampilkan seluruh informasi produk (SKU, nama produk, ukuran, dan jenis kain) secara lengkap untuk produk dengan beberapa jenis kain dalam satu ukuran. Menurunkan ukuran font caption menjadi `4.5pt` dengan tinggi baris `5.5pt`, serta meningkatkan kapasitas baris maksimum menjadi 5 baris (`maxCaptionLines = 5`) agar dapat menampung nama produk dan varian yang panjang dengan sempurna pada lebar 1.8cm tanpa ada info yang terpotong. Rincian spesifikasi: `doc/versions/v3.43.md`. |
| v3.42 | 2026-08-27 | Frontend | **IMPLEMENTED: Pembersihan Header & Tombol Tambah Melayang di Halaman Resep dan Produksi.** Menyederhanakan tampilan atas halaman resep (`ResepListView.swift`) dan produksi (`ProduksiBatchView.swift`) dengan menghapus baris judul header (`sectionHeader`) yang redundan karena sudah diwakili oleh segmen tab aktif di atasnya. Memindahkan tombol tambah bahan/produksi (+) dari header atas ke kanan bawah berupa tombol melayang kustom (`OuraFAB`) di atas daftar untuk visual modern, ergonomis, dan menghemat ruang vertikal. Rincian spesifikasi: `doc/versions/v3.42.md`. |
| v3.41 | 2026-08-27 | Frontend | **IMPLEMENTED: Pembersihan Header & Tombol Tambah Melayang di Halaman Bahan.** Menyederhanakan tampilan atas halaman bahan (`BahanListView.swift`) dengan menghapus baris judul "Bahan" (`sectionHeader`) yang redundan karena sudah diwakili oleh segmen tab "Bahan" aktif di atasnya. Memindahkan tombol tambah bahan (+) dari header atas ke kanan bawah berupa tombol melayang kustom (`OuraFAB`) di atas daftar bahan untuk visual modern, ergonomis, dan menghemat ruang vertikal. Rincian spesifikasi: `doc/versions/v3.41.md`. |
| v3.40 | 2026-08-27 | Frontend | **IMPLEMENTED: Penyelarasan Layout & Skala Teks Summary Header Produk.** Menyesuaikan kartu ringkasan statistik (Total Produk, Total Stok, Stok Kosong) di `ProdukListView.swift` agar caption/label judul tidak terpotong (truncated) pada perangkat layar sempit dengan menerapkan `.minimumScaleFactor(0.7)`, mengizinkan auto-wrap/penyesuaian font, dan merapikan serta merapatkan padding internal serta spacing antar kartu/capsule agar simetris dan solid. Rincian spesifikasi: `doc/versions/v3.40.md`. |
...
| v3.38 | 2026-08-26 | Frontend | **IMPLEMENTED: Perbaikan Error Decoding Analisis Waste Kain.** Mendiagnosis dan memperbaiki masalah *decoding crash* (`keyNotFound 'total_waste_area_cm2'`) pada halaman Laporan Waste Kain dengan mengubah tipe properti `totalWasteAreaCm2` di `ReportsModels.swift` menjadi opsional (`Double?`) serta menyematkan fallback default `?? 0` di `ReportsView.swift`. Rincian spesifikasi: `doc/versions/v3.38.md`. |
| v3.37 | 2026-08-26 | Frontend | **IMPLEMENTED: Pagination Berkinerja Tinggi (Infinite Scroll) di Halaman Produk.** Memperkenalkan pemuatan tak terbatas (*Lazy/Infinite Scroll Chunking*) pada `ProdukListView.swift`. Data produk dimuat secara bertahap (per 20 item) saat pengguna melakukan scroll mendekati akhir daftar. Ini menyelesaikan masalah lag rendering, sementara ringkasan statistik atas (Total Produk, Total Stok) tetap presisi 100% menggunakan akumulasi dataset. Rincian spesifikasi: `doc/versions/v3.37.md`. |
| v3.36 | 2026-08-26 | Frontend | **IMPLEMENTED: Optimasi Pemuatan Produk di Generator QR.** Memperbaiki kendala pemuatan lambat (*indefinite spinner*) saat membuka halaman Generator QR dengan mengganti perulangan sekuensial `api.getProductSizes(sku:)` yang boros panggilan jaringan (masalah N+1 HTTP requests) menggunakan satu pemanggilan tunggal `api.getAllProductSizes()` ter-agregasi. Rincian spesifikasi: `doc/versions/v3.36.md`. |
| v3.35 | 2026-08-26 | Frontend | **IMPLEMENTED: Penambahan Total Pcs Terjual di Laporan Penjualan.** Menambahkan akumulasi total kuantitas produk (*pcs*) terjual harian pada seksi Per Periode (misal: `26 transaksi · 35 pcs`) dan total keseluruhan pcs terjual selama rentang tanggal terpilih pada seksi Ringkasan. Rincian spesifikasi: `doc/versions/v3.35.md`. |
| v3.34 | 2026-08-26 | Frontend | **IMPLEMENTED: Penyesuaian Target Deployment ke iOS 16 & Perbaikan Kompilasi .onChange.** Menurunkan target sistem operasi (*deployment target*) aplikasi pada konfigurasi build `.pbxproj` dari nilai yang salah (`26.0`) menjadi `16.5` agar kompatibel dengan iOS 16+. Memperbaiki compile error merah pada modifier `.onChange` di file `BahanListView.swift` dengan menambahkan argument buangan `_ in` pada parameter perform closure-nya agar mematuhi aturan kontrak API iOS 16. Rincian spesifikasi: `doc/versions/v3.34.md`. |
| v3.33 | 2026-08-26 | Frontend | **IMPLEMENTED: Tampilan Preload Peringatan Stok di Dashboard.** Menambahkan skeleton loader/preload view berukuran presisi (`height: 46`, `width: 260`) di seksi Peringatan Stok BerandaView saat loading (`isLoading` bernilai true). Hal ini mencegah layout shift (pergeseran letak) yang membuat seksi Kasir Penjualan Kilat meloncat turun setelah data selesai dimuat. Rincian spesifikasi: `doc/versions/v3.33.md`. |
...
| v3.18 | 2026-08-20 | Frontend+Backend | **IMPLEMENTED (frontend) / PLANNED (backend): QR Cart Mode + HPP Fallback untuk Stok Manual.** Dua fitur sekaligus. (1) **QR Cart Mode:** `QRScannerSheet` mode `.sellOnly` kini pakai keranjang — scan bisa dilakukan berulang tanpa konfirmasi satu-satu; setiap scan menambah item ke `cartItems`, scanner tetap aktif; cart bar di bawah menampilkan item count + total; checkout dilakukan sekali via `QRCartCheckoutSheet` → satu `POST /sales-orders` multi-item + `markSalesOrderPaid`. Scan ulang QR yang sama increment qty; scan melebihi stok ditolak dengan toast. Payment method chips persist via `@AppStorage("qr_lastPaymentMethod")`. Warning banner tampil di checkout jika ada produk tanpa HPP batch. iOS 16 spring fix: `.spring(duration:)` → `.spring(response:dampingFraction:)`. (2) **QR PDF layout:** 1.8cm QR, 2mm margin, 0.5mm gap → 11 kolom per A4; 3-line caption (productName / fabricVariantName / sizeLabel) dengan font 5.5pt, forced 6pt line height via `NSMutableParagraphStyle`. (3) **HPP Fallback Opsi A:** `POST /sales-orders` saat ini reject jika product_size tidak punya HPP dari batch produksi. Solusi: backend fallback ke PatternSpec HPP (material cost + labor dari spec terbaru). Frontend: warning banner informatif di `ScanToSellSheet` dan `QRCartCheckoutSheet` ketika `latestHppBreakdown == nil` — tidak memblokir penjualan. Backend spec: `../backend/doc/implement-v3.18-hpp-fallback.txt`. Lihat `doc/versions/v3.18.md`. |
| v3.17 | 2026-08-19 | Frontend+Backend | **PLANNED: QR Code — Generate, Print, Scan Produk & Penjualan.** Tiga kapabilitas: (1) **QR Generator** di Tab Produk — pilih varian produk (multi-select), generate QR per varian via `CoreImage.CIQRCodeGenerator`, layout PDF A4 (3×3cm per QR, 6 kolom × 7 baris = 42 per halaman), print/simpan via `UIPrintInteractionController`. (2) **QR Scanner** universal via `DataScannerViewController` (VisionKit, iOS 16+) — decode string `oura:{productSizeId}`, resolve via `GET /product-sizes/{id}` (endpoint baru), tampilkan card konfirmasi + pilihan action. (3) **Fast-path post-scan** — `ScanToSellSheet` (qty + harga + metode bayar → `POST /sales-orders`) dan `ScanToStockSheet` (qty + alasan → `adjustStock`). Scanner diakses dari toolbar Tab Penjualan (mode sell), toolbar Tab Produk (mode stock-in), dan Beranda quick action "Scan & Jual". **Backend:** 1 endpoint baru `GET /product-sizes/{id}` — lookup ProductSizeDetail by UUID tanpa perlu SKU; query reuse logic dari `GET /products/{sku}/sizes`, tidak ada migration. Lihat `doc/versions/v3.17.md` dan `../backend/doc/versions/v3.17.md`. |
| v3.16 | 2026-08-19 | Frontend | **IMPLEMENTED: Varian Kain di TambahResepSheet.** `TambahResepSheet` saat ini selalu simpan semua kain ke satu PatternSpec per ukuran — benar untuk kombo (pouch) tapi salah untuk varian kain (scrunchie: tiap unit hanya butuh satu kain). Optimizer lalu menampilkan semua warna sebagai kebutuhan simultan. Fix: tambah selector "Tipe Resep" (Varian Kain default / Kombo Kain) identik dengan `TambahProdukLengkapSheet`. **Varian Kain save logic:** tiap fabric × tiap size → cari existing ProductSize dengan `(sizeLabel, fabricVariantName=fabric.name)`, jika tidak ada buat baru via `POST /products/{sku}/sizes?fabric_variant_name=...`; create PatternSpec dengan satu fabric saja. **Kombo Kain:** behavior existing tidak berubah. Tidak ada backend change — semua endpoint sudah ada sejak v1.7. FabricGroup tetap berguna: input sekali per family, saat save tiap warna dalam family jadi PatternSpec terpisah. Lihat `doc/versions/v3.16.md`. |
| v3.15 | 2026-08-18 | Frontend+Backend | **PLANNED (belum diimplementasikan): Fabric Family / Jenis Kain.** User punya puluhan material kain per warna (Satin Pink, Satin Merah, dst) — di form resep harus isi dimensi potongan per warna padahal semua warna satu jenis biasanya identik. Solusi: field `fabric_family` (nullable string) di `Material` untuk mengelompokkan kain sejenis. **Backend (required first):** migration `ALTER TABLE material ADD COLUMN fabric_family VARCHAR(100) NULL` + update `MaterialOut`/`CreateMaterialRequest`/`PatchMaterialRequest` + endpoint baru `GET /materials/families` (distinct families, sorted A-Z) — route /families harus didaftarkan sebelum /{material_id}. Tidak ada perubahan PatternSpec endpoint. **Frontend:** (1) `Material` model tambah `fabricFamily: String?` + CodingKey `fabric_family`; (2) field "Jenis Kain" editable di BahanDetailView + TambahPembelianSheet (autocomplete dari `GET /materials/families`); (3) `TambahResepSheet` + `TambahProdukLengkapSheet` step 2: picker tampilkan families bukan individual material; input dimensi per family per tab ukuran; expand family → individual materialId sebelum kirim ke PatternSpec API; per-material override tetap bisa via expand chevron. Tidak breaking — material tanpa family tetap tampil individual. Lihat `doc/versions/v3.15.md` dan `../backend/doc/implement-v3.15-fabric-family.txt`. |
| v3.14 | 2026-08-18 | Frontend | **IMPLEMENTED: Multi-size recipe tabs + Tipe Produk selector + XS–XXL preset.** Empat perubahan sekaligus. (1) **Multi-size recipe input:** `TambahResepSheet` dan `TambahProdukLengkapSheet` kini menampilkan tab per ukuran (XS, S, M, L, XL, XXL) dengan checkmark indicator bila dimensi semua kain sudah diisi; state diubah dari flat `[UUID: Double]` ke nested `[String: [UUID: Double]]` keyed by sizeLabel; `canSave` cukup butuh minimal satu ukuran lengkap; save loop semua completed sizes. (2) **NumericInputField stale fix:** tambah `onChange(of: value)` dengan guard `!isFocused` + `.id(activeSizeLabel + fabId.uuidString)` untuk force view recreation saat ganti tab. (3) **Tipe Produk selector di step 1:** menggantikan toggle "Gabungkan dalam satu resep" yang membingungkan; dua card di step 1 — "Varian Kain" (tiap kain jadi ProductSize tersendiri, default) dan "Kombo Kain" (semua kain satu spec); state `isFabricVariant: Bool`. (4) Preset "XS–XL" → "XS–XXL" + tombol "+ Ukuran" di tab row step 2 (auto-switch ke tab baru setelah tambah). Tidak ada backend change. Files: `TambahResepSheet.swift`, `TambahProdukLengkapSheet.swift`, `NumericInputField.swift`. Lihat `doc/versions/v3.14.md`. |
| v3.12 | 2026-08-17 | Frontend | **IMPLEMENTED: HPP Rincian + Price Advisor inline di Tab Produksi.** Lihat `doc/versions/v3.12.md` untuk detail lengkap. |
| v3.11 | 2026-08-15 | Frontend | **FIXED: Tambah Varian Kain tanpa resep tidak bisa simpan stok awal.** `AddSizeSheet` (dibuka via "+ Tambah Varian" di `ProdukSizeGroupView`) sebelumnya memblokir tombol "Simpan" ketika `selectedSpecId == nil` dan dimensi potongan kosong — sehingga produk yang dibuat tanpa resep ("Simpan Tanpa Resep") tidak bisa menambah stok awal sama sekali karena user hanya bisa pilih varian kain tanpa bisa simpan qty. **Root cause:** `canSave` mensyaratkan `manualCutWidthCm` + `manualCutLengthCm` ketika tidak ada specId dan qty > 0; `save()` juga tidak punya else-branch untuk kasus tanpa spec dan tanpa dimensi. **Fix — tiga path stok awal sekarang terdefinisi:** (1) Ada resep (`specId != nil`) → `addStockFromBahan` — stok bahan ikut dikurangi sesuai resep; (2) Tidak ada resep tapi dimensi diisi → `addStockManual` — stok bahan dikurangi sesuai dimensi manual; (3) Tidak ada resep dan dimensi kosong → `adjustStock(reason: "initial")` — stok ditambah manual, bahan tidak disentuh. Dimensi potongan dijadikan opsional (bukan wajib). Footer text "Stok Awal" sekarang menampilkan tiga state berbeda sesuai kondisi. `canSave` untuk kasus "size already exists" disederhanakan: cukup `qty > 0`, tidak lagi blokir jika tidak ada spec. |
| v3.10 | 2026-08-15 | Frontend | **FIXED: Tiga menu laporan di tab Lainnya sekarang berfungsi — dua root cause.** **(1) Format tanggal salah (422):** `getSalesReport` dan `getWasteByMaterial` di `APIService.swift` menggunakan `ISO8601DateFormatter()` yang menghasilkan `2026-08-15T14:30:51Z` — backend mendeklarasikan `from`/`to` sebagai tipe `date` (bukan `datetime`) dan mengembalikan HTTP 422 "Datetimes provided to dates should have zero time" untuk setiap request. Diganti ke `DateFormatter` dengan format `yyyy-MM-dd` (shared property `dateFmt`, `locale: en_US_POSIX`). **(2) Codable model mismatch (keyNotFound):** backend mengembalikan field dengan nama berbeda dari yang iOS modelkan. Perbaikan per model: `SalesReportPoint` — backend kirim `"date"` bukan `"period"` (sudah diperbaiki via `case period = "date"`); backend pakai camelCase (`"totalRevenue"`, `"totalProfit"`, `"orderCount"`) bukan snake_case — hapus CodingKey overrides yang salah. `SalesReport` root — sama, camelCase, hapus CodingKeys seluruhnya. `MarginRankingItem` — backend kirim `"hpp_total"` bukan `"hpp"`, tidak ada field `"id"` (pakai `product_size_id`), tidak ada `markup_pct`; tambah `fabricVariantName`. `WasteByMaterial` — backend tidak kirim `"id"`, pakai `material_id` sebagai identitas. **MockAPIService:** setelah stored `id` dihapus dari ketiga struct, call site mock masih pass argumen `id: UUID()` dan `markupPct:` — dihapus. **(3) UX improvement:** `SalesReportDetailView` sebelumnya pakai `try?` yang menyembunyikan semua error — screen tampil kosong tanpa pesan apapun. Diganti ke `do-catch` dengan `@State private var errorMsg` — error state eksplisit (segitiga kuning + pesan) + empty state terpisah ("Belum ada transaksi di periode ini"). Lihat commits `00e48cf` (mock fix), `d80eedf` (date format + error state), `658136b` (CodingKey fix). |
| v3.9 | 2026-08-15 | Frontend+Backend | **DECIDED: Benang (thread) = Model A (pooled flat rate).** Tiga layer tidak konsisten: (1) Settings: `pooled_material_rate:benang` (Rp/pcs); (2) Pembelian Bahan: per gulung, tanpa length tracking; (3) Resep: beberapa material benang masuk sebagai Bahan Tambahan dengan `usage_unit = "cm2"` — unit salah dan tidak seharusnya ada di resep (akan double-count dengan pooled rate). **Keputusan:** benang tetap pooled — biayanya kecil, konsumsi per produk sulit diukur secara akurat, tracking presisi tidak sebanding effort. **Implikasi:** (a) benang tidak boleh menjadi PatternComponent — hapus dari resep yang ada untuk menghindari double-count; (b) set nilai `pooled_material_rate:benang` di Settings sesuai estimasi owner; (c) pembelian benang tetap dicatat di tab Bahan untuk tracking pengeluaran, tidak di-deduct per batch produksi; (d) HPP benang masuk ke `hpp_pooled_material`. Tidak ada perubahan kode — arsitektur pooled sudah berjalan sejak awal. Jika di kemudian hari ingin presisi per produk → Model B: direct_precise component dengan `usage_unit = "cm"` di resep, tanpa pooled. |
| v3.8 | 2026-08-14 | Frontend+Backend | **IMPLEMENTED: HPP breakdown per material — fabric_items + hardware_items di HPPBreakdown.** Saat ini `HPPBreakdown` hanya menyimpan angka agregat (fabric: Rp3.025, hardware: Rp0). Untuk produk multi-kain seperti Pouch, angka ini tidak menjelaskan kontribusi per kain. **Backend (required first):** tambah `fabric_items: [{name, cost}]` dan `hardware_items: [{name, cost}]` ke shape `HppBreakdownOut` di `GET /products/{sku}/sizes` dan `GET /products/{sku}/sizes/{id}`. `fabric_items` di-derive dari `production_batch_layout → cutting_layout → cutting_layout_item → material_purchase → material.name` + `cost_per_piece` per-layout (historical, akurat). `hardware_items` di-derive dari `pattern_spec.components` × `material.current_avg_cost` (approx — noted in UI). Tidak ada schema migration. **Frontend model:** tambah `HPPLineItem: Codable { name: String, cost: Double }` struct baru; tambah `fabricItems: [HPPLineItem]` (`"fabric_items"`) dan `hardwareItems: [HPPLineItem]` (`"hardware_items"`) ke `HPPBreakdown` dengan default `?? []` untuk backward compat. **Frontend UI (`ProdukSizeDetailView`):** di section HPP Breakdown, tampilkan indented sub-rows di bawah baris "Kain" dan "Hardware" — hanya ketika `items.count > 1` (1 item = aggregate sudah cukup). Sub-row style: font 12pt, `textSecondary`, leading +16pt, tanpa persen. Lihat `backend/doc/implement-v3.8-hpp-breakdown-per-material.txt` untuk spec backend. |
...
| v3.7 | 2026-08-13 | Frontend | **OptimasiView redesign: flow mulai dari produk, bukan dari kain.** Flow lama ("pilih gulungan kain → filter kandidat") tidak scalable untuk produk multi-kain: Pouch dengan 3 kain tidak muncul sebagai kandidat karena entry point arbitrer pada satu roll. Flow baru: **(1) Step 1 — Pilih Produk:** daftar semua PatternSpec aktif dikelompokkan per nama produk (e.g. "Pouch" → S, M, L); setiap row menampilkan ukuran, nama kain-kain yang dibutuhkan, dan badge "N kain" untuk multi-fabric. **(2) Step 2 — Pilih Kain:** setelah produk dipilih, sistem render satu roll picker per fabric dari `selectedSpec.fabrics`; setiap picker hanya tampilkan gulungan dengan `materialId` yang sesuai; `canCalculate` = semua fabric sudah punya roll. **(3) Step 3 — Hasil:** sama seperti sebelumnya — bottleneck panel + save N CuttingLayouts + 1 ProductionBatch. **State dihapus:** `selectedPurchase`, `selectedMaterial`, `candidates: [UUID:Int]`, `otherFabricIds`, `otherFabricRolls`, `filteredSpecs`. **State baru:** `selectedSpec: PatternSpec?`, `rollSelections: [UUID: MaterialPurchase]` (keyed by materialId). `calculateLayouts()` disederhanakan: satu `OptimizerCandidate` per session (spec yang dipilih), N API call per fabric. Tidak ada backend change. Semua perubahan di `OptimasiView.swift`. |
| v3.6 | 2026-08-13 | Frontend | **DONE: Optimasi multi-kain — bottleneck flow + multi-layout batch.** Saat ini OptimasiView hanya bisa optimalkan satu roll per sesi. Untuk Pouch XXL dengan 3 kain, user harus run 3 sesi terpisah dan menghitung bottleneck manual. **Perubahan yang direncanakan:** (1) **ProductionModels.swift:** `BackendProductionBatch` + `ProductionBatch` tambah `cuttingLayoutIds: [UUID]` (ordered); `CreateProductionBatchRequest` ganti `cuttingLayoutId: UUID?` ke `cuttingLayoutIds: [UUID]`. (2) **APIService.createProductionBatch:** parameter berubah ke `cuttingLayoutIds: [UUID]`, kirim array ke backend. (3) **OptimasiView Step 2 — Pilih Pola:** deteksi kandidat multi-kain (`spec.fabrics.count > 1`). Jika ada, tampilkan section "Kain Lainnya" — satu row per fabric yang tidak dicovered roll utama Step 1, masing-masing punya roll picker. State baru: `otherFabricRolls: [UUID: MaterialPurchase]` (materialId ke purchase). (4) **OptimasiView — Hitung Optimasi:** run `suggestLayouts` N kali in parallel (satu per fabric roll). State baru: `layoutsPerFabric: [UUID: [OptimizerLayout]]`. (5) **OptimasiView Step 3 — Hasil:** tampilkan bottleneck panel — "Maksimal X pcs (bottleneck: Nama Kain)" dihitung dari min totalQty across all fabrics untuk strategy yang sama. (6) **Gunakan Layout Ini:** buat N CuttingLayout (satu per fabric), kumpulkan IDs, lalu `createProductionBatch(cuttingLayoutIds:)` — satu batch mencakup semua kain. (7) **MockAPIService.createProductionBatch:** terima `cuttingLayoutIds: [UUID]`, aggregate items dari semua layouts, bottleneck qty, sum fabric costs. Single-fabric flow tidak berubah. Lihat backend v2.16 dan `implement-v2.16-multi-layout-batch.txt` untuk schema + endpoint yang harus selesai dulu. |
| v3.5 | 2026-08-12 | Frontend+Backend | **Multi-fabric PatternSpec — satu resep untuk semua kain (frontend DONE, backend v2.15 PLANNED).** Arsitektur lama menyimpan satu PatternSpec per kain via toggle "Gabungkan/Pisah". Ini menyebabkan tiga bug fundamental untuk produk multi-kain: stok triple-counted, HPP tidak lengkap, optimizer buta bottleneck. **Arsitektur baru (frontend implemented):** satu PatternSpec = satu resep lengkap untuk satu ukuran produk, berisi N fabric layers. **Frontend changes:** (1) `PatternSpecModels.swift`: `BackendPatternSpec` ganti flat fields → `fabrics: [BackendPatternFabric]` array; `BackendCreatePatternSpecRequest` ganti flat fields → `fabrics: [FabricLayerRequest]` array; baru: `BackendPatternFabric` struct. (2) `APIService.enrichPatternSpecs`: baca `spec.fabrics[]` dari backend (material name sudah embedded, tidak perlu lookup terpisah); `usedInBatchCount` kini diambil dari response (sebelumnya hardcoded 0). (3) `APIService.createOrUpdatePatternSpec`: kirim semua kain dalam satu POST. (4) `TambahResepSheet` + `ResepEditorView`: hapus `gabungkanResep` toggle + state + kedua cabang save; satu POST dengan semua kain terpilih. (5) `ResepEditorView.startEdit()`: pre-populate dari `representative.fabrics[]` (bukan per-spec). **Backend PLANNED (v2.15):** tabel join `pattern_spec_fabric`, migration data, optimizer filter update — lihat `backend/doc/handoff.md` v2.15 dan prompt `backend/doc/implement-v2.15-multi-fabric-pattern-spec.txt`. |
| v3.4 | 2026-08-11 | Frontend | **TambahProdukLengkapSheet: "Dari Resep" discovery + TambahStokDariResepSheet.** Step 1 di `TambahProdukLengkapSheet` kini menampilkan section "DARI RESEP" sebelum form buat produk baru. Section ini load `getPatternSpecs()` + `getProductSizes(sku:)` per-produk on-appear; produk dengan total stok = 0 ditampilkan prominent (badge kuning "Belum ada stok"), produk dengan stok ada ditampilkan dimmed (badge abu "Stok: X pcs"). Tapping produk membuka `TambahStokDariResepSheet` — sheet stepper per-ukuran yang memanggil `POST /products/{sku}/sizes/{sizeId}/stock-adjustments` (`reason: "initial"` jika stok 0, `"adjustment"` jika sudah ada). Setelah simpan, list DARI RESEP di-refresh otomatis. Tidak ada perubahan backend — endpoint stock-adjustments sudah ada sejak v2.0. Struct baru: `ResepProductInfo` (private), `TambahStokDariResepSheet` (private). |
| v3.3 | 2026-08-11 | Frontend | **ResepListView: grouping by productSizeId + ResepEditorView: multi-spec group edit.** `ResepListView` kini group PatternSpec by `productSizeId` — specs gabungkan (share satu ProductSize) tampil sebagai SATU baris dengan semua nama kain digabung ("Waffle Blue · Waffle Red · Waffle Hijau"); specs pisah (masing-masing punya ProductSize sendiri) tetap 1 row terpisah + dimension tag. `ResepEditorView` diubah init dari `spec: PatternSpec` ke `specs: [PatternSpec]` — membuka semua spec satu group sekaligus; display mode menampilkan semua kain bersama; edit mode pre-populate semua fabric dari semua spec; on-save update/create per fabric + delete spec untuk kain yang dihapus user (jika `canDelete`). `SpecGroup` struct baru di `ResepListView.swift`. Tidak ada perubahan backend/model. |
| v3.2 | 2026-08-10 | Frontend | **Rencana: Tambah Item ke Batch Produksi Manual (belum diimplementasikan).** Batch manual ("+" di tab Produksi tanpa optimizer) saat ini selalu kosong — tidak ada cara menambah item. Rencana: (1) tombol "+ Tambah Item" di `BatchCard` untuk draft batch manual; (2) `TambahItemBatchSheet` — pilih product_size + qty, multi-item, sequential POST; (3) swipe-delete item dari draft batch. Backend butuh 2 endpoint baru: `POST /production-batches/{id}/items` + `DELETE /production-batches/{id}/items/{item_id}` (lihat backend v2.14). Tidak ada perubahan skema. Lihat `doc/versions/v3.2.md`. |
| v3.1 | 2026-08-10 | Frontend | **Tambah Produk Lengkap: produk + resep dalam satu alur two-step.** FAB "+" di tab Produk kini membuka `TambahProdukLengkapSheet` — step 1 mengisi nama/SKU/ukuran, step 2 mengisi resep (kain multi-select + dimensi, komponen, labor). Keduanya optional: "Simpan Tanpa Resep" tersedia di step 1 dan step 2. Multi-ukuran: resep hanya untuk satu ukuran yang dipilih (ukuran lain diresep dari tab Resep). Save logic identik dengan `TambahResepSheet`. `AddProductSheet` (private struct lama) dihapus. Tidak ada perubahan backend. See Section 2, Screen 3 (Products). |
