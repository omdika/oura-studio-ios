...
|---|---|---|---|
| v3.61 | 2026-09-26 | Frontend | **IMPLEMENTED: Sertakan Harga di Label QR (Toggle + Posisi di Atas Size).** Menambahkan opsi "Sertakan Harga" (radio Ya/Tidak) di Generator QR dan di Pengaturan Printer Thermal (tersinkron via AppStorage), mencetak harga di atas caption size pada label thermal 33×15mm & PDF A4. Rincian spesifikasi: `doc/versions/v3.61.md`. |
| v3.60 | 2026-09-19 | Frontend | **PLANNED: Total Nilai Stok di Halaman Produk (Nilai Modal + Potensi Omset).** Menambahkan banner full-width di bawah 3 card ringkasan ProdukListView yang menampilkan Nilai Modal (stok × HPP efektif, fallback harga jual) dan Potensi Omset (stok × harga jual), menghormati search & filter tanggal, frontend-only tanpa perubahan backend. Rincian spesifikasi: `doc/versions/v3.60.md`. |
...
| v3.54 | 2026-09-13 | Frontend | **PLANNED: Summary Penjualan (Total Omset, Total Pcs, Total Transaksi) & Flag Dibatalkan.** Menambahkan 3 card ringkasan di bawah DateRange field pada PenjualanListView untuk menampilkan Total Pendapatan, Total Pcs Terjual, dan Total Transaksi. Transaksi berstatus `cancelled` diabaikan dari perhitungan statistik namun tetap muncul di daftar penjualan dengan visual flag "Dibatalkan" yang jelas. Rincian spesifikasi: `doc/versions/v3.54.md`. |
| v3.53 | 2026-09-13 | Frontend | **IN_PROGRESS: Export JSON Laporan Penjualan untuk Analisa AI.** Menambahkan tombol "Export JSON untuk AI" di SalesReportDetailView yang men-compose data dari 3 endpoint existing (sales-orders, reports/sales, reports/sales-by-product) menjadi satu file JSON self-contained berisi export_info, summary agregat (revenue/COGS/profit/breakdown per payment method/daily trend/top products), dan seluruh raw orders dengan detail item (HPP, margin, diskon). Client-side composition, zero backend changes. Rincian spesifikasi: `doc/versions/v3.53.md`. |
| v3.52 | 2026-09-12 | Frontend | **PLANNED: Direct TSPL Thermal Label Printing.** Implementasi pencetakan label QR Code secara langsung ke printer thermal roll via CoreBluetooth menggunakan perintah TSPL, dengan dimensi label 33x15mm dan gap 2mm yang dapat dikonfigurasi. Rincian spesifikasi: `doc/versions/v3.52.md`. |
| v3.51 | 2026-09-07 | Fullstack | **REFINED/PLANNED: Shopee Bulk Upload Excel Generator.** Redesain alur ekspor menggunakan mekanisme duplikasi file master template resmi Shopee (`Shopee_mass_upload_2026-09-07_basic_template.xlsx`) dan menyunting mulai baris 7 di sheet 'Template' agar lolos verifikasi parser internal Shopee Seller Centre. Rincian spesifikasi: `doc/versions/v3.51.md`. |
...
| v3.49 | 2026-09-04 | Fullstack | **IMPLEMENTED: Integrasi Native Google Sign-In SDK.** Merombak metode login Google SSO dari ASWebAuthenticationSession dengan menggunakan Google Sign-In SDK resmi untuk iOS secara native, mengaktifkan flow otentikasi aman melalui GIDSignIn, dan menyalurkan openURL callback langsung ke SDK. Rincian spesifikasi: `doc/versions/v3.49.md`. |
