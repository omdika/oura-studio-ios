...
struct QRScannerSheet: View {
    ...
    // Manual product search (NEW)
    @State private var showManualProductPicker = false
    @State private var availableProductSizes: [ProductSizeDetail] = []
    @State private var isLoadingProducts = false

    private var isScanning: Bool {
        scanState == .scanning && !showCheckoutSheet && !showManualProductPicker // Pause scanner when manual picker is open
    }
    ...
    // Derived property for ProductPickerSheet
    private var selectedSizeIdsInCart: Set<UUID> {
        Set(cartItems.map { $0.size.id })
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar {
                ...
                // NEW: Toolbar item for manual product search
                ToolbarItem(placement: .primaryAction) {
                    if mode == .sellOnly { // Only show manual search for selling mode
                        Button {
                            showManualProductPicker = true
                        } label: {
                            Image(systemName: "magnifyingglass")
                        }
                        .foregroundStyle(OuraTheme.Colors.accent)
                        .accessibilityLabel("Cari Produk Manual")
                    }
                }
            }
        }
        ...
        // NEW: Sheet for manual product picker
        .sheet(isPresented: $showManualProductPicker, onDismiss: {
            // Resume scanning if the picker is dismissed and not in another state
            if scanState != .resolving && scanState != .resolved(.dummy) && scanState != .error("") {
                scanState = .scanning
            }
        }) {
            ProductPickerSheet(
                sizes: availableProductSizes,
                alreadySelected: selectedSizeIdsInCart // Pass currently selected items in cart
            ) { selectedSize in
                addToCart(selectedSize) // Reuse existing addToCart logic
            }
        }
        .task {
            await loadAvailableProductSizes() // Load products for manual picker
        }
    }
    ...
    // NEW: Function to load all product sizes for the manual picker
    private func loadAvailableProductSizes() async {
        isLoadingProducts = true
        defer { isLoadingProducts = false }
        do {
            let all = try await api.getAllProductSizes()
            availableProductSizes = all.filter { !$0.isArchived && $0.currentStockQty > 0 }
        } catch {
            print("Error loading all product sizes for manual picker: \(error.localizedDescription)")
